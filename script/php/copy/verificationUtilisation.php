...
if(isset($_POST['selector'])){
    ...
    // verification de l'utilisation des information de l'exemplaire
    while($champ = $request -> fetch()){
        ...
        for($i=0;$i<count($selector);$i++){
            ...
            // verification de l'utilisation de l'exemplaire
            if($selector[$i] == $champ["CODEEXEMPLAIRE"]){
                $codeErreur['code'] = 2;
                $message .= $champ["CODEEXEMPLAIRE"]."<br/>";
            }
        }
    }
    ...
    if($codeErreur['code']==2){
        $codeErreur['message']="les exemplaires suivantes ne peuvent etre supprimer: </br>";
